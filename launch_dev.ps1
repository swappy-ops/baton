# Baton Startup Orchestrator

Write-Host "🚀 Initializing Baton Neural Observatory..." -ForegroundColor Cyan

# 1. Start Backend
Write-Host "[1/4] Starting Backend Server..." -ForegroundColor Gray
Start-Process -NoNewWindow -FilePath "uvicorn" -ArgumentList "baton_server.main:app --reload --port 8000"
...
Set-Location baton_ui
Start-Process -NoNewWindow -FilePath "npm" -ArgumentList "run dev"
Set-Location ..

# 4. Verification
Write-Host "[4/4] Verifying System Readiness..." -ForegroundColor Gray
python baton_server/services/health_check.py

Write-Host "`n✨ Baton is now ONLINE." -ForegroundColor Green
